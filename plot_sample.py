...
with torch.no_grad():
    ...
    for data in test_generator:
        ...
        for i in range(4):
            ax[i].plot(mean_results[0,:,0], mean_results[0,:,i+1])
            ax[i].fill_between(mean_results[0,:,0], mean_results[0,:,i+1]-std_results[0,:,i+1], mean_results[0,:,i+1]+std_results[0,:,i+1], alpha = 0.5)
            ax[i].plot(times[0][mask[0, :,i]>0].cpu(), y[0,:,i][mask[0, :,i]>0].cpu(), marker = 'o', color = 'r', linestyle='')
