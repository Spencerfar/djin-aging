...
from Alternate_models.model_1d import Model

from DataLoader.dataset import Dataset
from DataLoader.collate import custom_collate
from Utils.schedules import LinearScheduler

from Alternate_models.loss_full import loss, sde_KL_loss, vae_loss
...
loss_file = '%svalidation%d_1d.loss'%(output_folder, args.job_id)
open(loss_file, 'w')

# output hyperparameters
hyperparameters_file = '%strain%d_1d.hyperparams'%(output_folder, args.job_id)
...
model = Model(device, N, args.gamma_size, args.z_size, args.decoder_size, mean_T, std_T, dt).to(device)
...
for epoch in range(niters):
    ...
    for data in training_generator:
        ...
        pred_X, pred_Z, t, pred_S, pred_logGamma, pred_sigma_X, context, y, times, mask, survival_mask, dead_mask, after_dead_mask, censored, sample_weights, med, env, mu0, logvar0, recon_mean_x0, mask0 = model(data, sigma_y)
        summed_weights = torch.sum(sample_weights)
        
        kl_term = torch.sum(torch.sum(sample_weights*((mask*sigma_posterior.log_prob(sigma_y)).permute(1,2,0)),dim=(1,2)) - torch.sum(sample_weights*((mask*sigma_prior.log_prob(sigma_y)).permute(1,2,0)),dim=(1,2))) + beta_vae*vae_loss(mu0, logvar0, sample_weights)
        ...
        l.backward()
        nn.utils.clip_grad_norm_(model.parameters(), 1E6)
        optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i in range(test_average):
                ...
                for data in validation_generator:
                    ...
                    pred_X, pred_Z, t, pred_S, pred_logGamma, pred_sigma_X, context, y, times, mask, survival_mask, dead_mask, after_dead_mask, censored, sample_weights, med, env, mu0, logvar0, recon_mean_x0, mask0 = model(data, sigma_y, test=True)
                    summed_weights = torch.sum(sample_weights)
                    
                    
                    kl_term = torch.sum(torch.sum(sample_weights*((mask*sigma_posterior.log_prob(sigma_y)).permute(1,2,0)),dim=(1,2)) - torch.sum(sample_weights*((mask*sigma_prior.log_prob(sigma_y)).permute(1,2,0)),dim=(1,2))) + vae_loss(mu0, logvar0, sample_weights)
    ...
    # output params
    if epoch % 20 ==0:
        torch.save(model.state_dict(), '%strain%d_Model_1d_epoch%d.params'%(params_folder, args.job_id, epoch))
