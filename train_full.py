...
hyperparameters_file = '%strain%d_full.hyperparams'%(output_folder, args.job_id)
...
model = Model(device, N, args.gamma_size, args.z_size, args.decoder_size, args.Nflows, args.flow_hidden, mean_T, std_T, dt).to(device)
...
for epoch in range(niters):
    ...
    # output params
    if epoch % 20 ==0:
        torch.save(model.state_dict(), '%strain%d_Model_full_epoch%d.params'%(params_folder, args.job_id, epoch))
