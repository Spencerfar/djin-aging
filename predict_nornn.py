...
parser.add_argument('--imputation', default=True, action='store_false')
...
model.load_state_dict(torch.load('Parameters/train%d_Model_DJIN_nornn_epoch%d.params'%(args.job_id, args.epoch),map_location=device))
...
with torch.no_grad():
    ...
    W_posterior = torch.distributions.normal.Normal(model.mean, model.logscale.exp())
...
np.save('Analysis_Data/Mean_trajectories_job_id%d_epoch%d_nornn.npy'%(args.job_id, args.epoch), mean_results)
np.save('Analysis_Data/Std_trajectories_job_id%d_epoch%d_nornn.npy'%(args.job_id, args.epoch), std_results)
np.save('Analysis_Data/Survival_trajectories_job_id%d_epoch%d_nornn.npy'%(args.job_id, args.epoch), S_results)
